...
my $totaloutput = Documents->New("00crystal_refdata.txt");# the output of PSO referend data 
...
######### END OF DFT SETTING
my $docb1ref = $Documents{"CsCl.xsd"};
...
for (my $i =0; $i < $atomtypeNo - 1 ; $i++) {
  for (my $j = $i+1; $j < $atomtypeNo ; $j++){
     ...
     my $adjustlattice = 1.15*($pair1rad+$pair2rad); ###assumed lattice constant of b1 (a little larger than 2 times 
...
#($Alloy,$lmpE,$Bulk,$Volume,$Re,$C11,$C12,$C13,$C33,$C44);
    ...
    $xsd -> Export("$tempname"."_Fine.car");
    $xsd->SaveAs("$tempname"."_Fine.xsd");
    
    $Re= $xsd->UnitCell->Lattice3D->LengthA;# two atoms in each dimension for B1
	$Re = sqrt(3*($Re**2))/2; #accroding reference structure
   ...
   my $elastic = Modules->CASTEP->ElasticConstants->Run($elastic_docin);#,#Settings(
   
                                          # CalculateElasticConstants =>"Yes"));   
   $Bulk = $elastic ->BulkModulus;
   $C11 = $elastic ->C11;
   $C12 = $elastic ->C12;
   $C13 = $elastic ->C13;
   $C33 = $elastic ->C33;
   $C44 = $elastic ->C44;          
#($Alloy,$lmpE,$Bulk,$Volume,$Re,$C11,$C12,$C13,$C33,$C44);
   $totaloutput->Append(sprintf "$tempname,$lmpE,$Bulk,$Volume,$Re,$C11,$C12,$C13,$C33,$C44\n");
