...
!use omp_lib
implicit none
Integer*8:: i,j,k,cfgnum,natom,totalnatom,naveragenum
Integer typenum,nn
Integer,allocatable:: tt(:)
Integer*8,allocatable:: ncount(:),type(:)
real*8,allocatable:: allcoordx(:),allcoordy(:),allcoordz(:),disp(:)
real*8,allocatable:: coordx(:,:),coordy(:,:),coordz(:,:)
...
allocate(allcoordx(totalnatom),allcoordy(totalnatom),allcoordz(totalnatom),type(totalnatom))
