...
sub check_hw
{
    ...
    if($Ch == 0)                                                           
    {
        ...
        open input, "<input.txt"; #or die ("Can't find file");
        @input = <input>;
        close input;

        open output, "<output.txt"; #or die ("Can't find file");
        @output = <output>;
        close output;       
        ...
        for $n(0..30)
        {
            ...
            #print "2:$hwouts[$n]\n";
            #print "3:$output[$n]\n";
            if ($hwouts[$n] =~ /$input[$n]/ && $output[$n] =~ /$input[$n]/)
            {
                $HG = $HG + 1;
                #print "$HG\n";
            }
        ...
        else
        {
            ...
            print errorout "input:\n";
            foreach $inputf(@input){print errorout "$inputf\n";}
            print errorout "output:\n";
            foreach $outputf(@output){print errorout "$outputf\n";}
            print errorout "print:\n";
            print errorout "$hwout\n";

            #print errorout "First error $hwouts[$en[0]]\n\n";
            print "Homework Bad!\n";
